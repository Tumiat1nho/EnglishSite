    ...
    <div className="max-w-6xl mx-auto px-4 py-12">
      <div className="mb-8">
        ...
        <p className="text-gray-500 max-w-2xl">
          Amplie seu vocabulário em inglês com categorias temáticas. Cada categoria
          inclui pronúncia, tradução e exemplos de uso.
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-8">
        <div className="flex flex-wrap gap-2">
          ...
          {levelOrder.map((level) => {
            const count = vocabularyCategories.filter((v) => v.level === level).length;
            return (
              <button
                key={level}
        ...
        <div className="relative sm:ml-auto">
          <input type="text" placeholder="Buscar vocabulário..." value={search} onChange={(e) => setSearch(e.target.value)}
      ...
      {selectedLevel === "all" ? (
        levelOrder.map((level) => {
          const levelVocab = filtered.filter((v) => v.level === level);
          ...
          return (
            <section key={level} className="mb-12">
              <h2 className="text-xl font-bold text-gray-800 mb-4 flex items-center gap-2">
                ...
                {levelNames[level]}
                <span className="text-sm font-normal text-gray-400">({levelVocab.length})</span>
              </h2>
                ...
                {levelVocab.map((cat) => (
                  <Link key={cat.slug} href={`/vocabulario/${cat.slug}`} className="group">
                    <div className={`bg-white rounded-xl shadow-md hover:shadow-lg transition-all duration-300 p-6 border border-gray-100 group-hover:border-primary/20 group-hover:-translate-y-1 level-${level.toLowerCase()}`}>
                      ...
                      <h3 className="text-lg font-semibold mt-3 mb-2 text-gray-900 group-hover:text-primary transition-colors">{cat.title}</h3>
                      <p className="text-gray-500 text-sm leading-relaxed">{cat.description}</p>
                      <p className="text-xs text-gray-400 mt-3">{cat.words.length} palavras</p>
                    </div>
                  </Link>
                ))}
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filtered.map((cat) => (
            <Link key={cat.slug} href={`/vocabulario/${cat.slug}`} className="group">
              <div className={`bg-white rounded-xl shadow-md hover:shadow-lg transition-all duration-300 p-6 border border-gray-100 group-hover:border-primary/20 group-hover:-translate-y-1 level-${cat.level.toLowerCase()}`}>
                <div className="text-3xl mb-3">{cat.icon}</div>
                <LevelBadge level={cat.level} />
                <h3 className="text-lg font-semibold mt-3 mb-2 text-gray-900 group-hover:text-primary transition-colors">{cat.title}</h3>
                <p className="text-gray-500 text-sm leading-relaxed">{cat.description}</p>
